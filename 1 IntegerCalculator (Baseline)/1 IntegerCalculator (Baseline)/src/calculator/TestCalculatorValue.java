...
//		test = new CalculatorValue(t);										// Perform the test
		...
		// 5. Perform addition
		CalculatorValue left = new CalculatorValue("5");						// Set up the test
		CalculatorValue right = new CalculatorValue("5");
		...
		System.out.println();
		//testcase:2 for addition 
		
		 left = new CalculatorValue("3");						// Set up the test
		 right = new CalculatorValue("-6");
		...
		System.out.println();
		//testcase:3 for addition 
		
				 left = new CalculatorValue("-5");						// Set up the test
				 right = new CalculatorValue("-5");
...
//				left.div(right);														// Perform the test
				...
				// Check the actual output against the expected.  If they match, the test has been passed and display the proper
				// message and tally the result
				if (check("measuredValue = 0\nerrorMessage = Error: Division not possible\n", left.debugToString())) {
					numPassed++;
					System.out.println("\tPass");
				}
				// If they do not match, display that there was a failure and tally that result
